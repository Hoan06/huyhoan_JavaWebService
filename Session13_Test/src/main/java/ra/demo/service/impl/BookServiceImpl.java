...
public class BookServiceImpl implements BookService {
    ...
    public List<Book> getBooks() {
        log.debug("Gửi yêu cầu đến lấy danh sách ");

        List<Book> books = bookResporitory.findAll();

        log.info("Lấy danh sách thành công, tổng : {}", books.size());
        return books;
    }

    @Override
    public Book getBookById(Long id) {
        log.debug("Yêu cầu xem sách có id : {}", id);

        return bookResporitory.findById(id)
                .map(book -> {
                    log.info("Lấy sách thành công với id: {}", id);
                    return book;
                })
                .orElseThrow(() -> {
    ...
    public Book insertBook(BookDTO bookDTO) {
        log.debug("Yêu cầu thêm sách mới : {}", bookDTO);
        Book book = Book.builder()
                ...
                .build();
        Book saveBook =  bookResporitory.save(book);
        log.info("Thêm sách thành công với id: {}", saveBook.getId());
        return saveBook;
    }
